# Installs/updates the Windows Scheduled Task for SQL -> Active Directory password reset jobs.
...
param(
  ...
  [string]$WrapperPath = "C:\ZimmetAD\Run-ADPasswordAgentHidden.vbs",
  [int]$Limit = 5,
  [switch]$Visible,
  [switch]$WhatIfOnly
)
...
$arguments = "-NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $quotedScript -Limit $Limit"
if ($WhatIfOnly) {
  $arguments += " -WhatIfOnly"
}
